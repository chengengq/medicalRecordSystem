<template>
  <!-- 人才导出 -->
  <div class="main">
    <div class="times">
      <div class="times_wrap">
        <!-- 相关按钮 计算 导出 -->
        <div class="button_out">
          <el-button type="success" size="medium" @click="handleExports"
            >模板导出</el-button
          >
        </div>
        <div class="button_out">
          <el-upload
            ref="upload"
            class="upload-demo"
            :action="ipAddress"
            :headers="headers"
            :on-success="upSuccess"
            :on-remove="handleRemove"
            :data="{ month: childmonth, selectType: 2 }"
            multiple
            :limit="1"
            :file-list="fileList"
            name="file"
          >
            <el-button size="medium" type="success">文件上传</el-button>
          </el-upload>
        </div>
        <div class="tips">
          导入excel文件格式严格按照模板格式，否则会解析失败
        </div>
      </div>
    </div>
    <div class="main_wrap">
      <el-table style="width: 100%" border max-height="600" :data="tableList">
        <el-table-column
          fixed
          align="center"
          label="序号"
          type="index"
          width="70"
        >
        </el-table-column>
        <el-table-column
          fixed
          align="center"
          prop="items"
          label="项目名称"
          width="300"
        >
        </el-table-column>
        <el-table-column
          fixed
          align="center"
          prop="value_1"
          label="人数"
          width="80"
        >
        </el-table-column>
        <el-table-column align="center" label="已通过验收并参加排班的人员">
          <el-table-column
            v-for="(item, index) in tableTitle"
            :key="index"
            align="center"
            :label="String(index + 1)"
            :prop="'value_' + (index + 2)"
            width="80"
          >
          </el-table-column>
        </el-table-column>
      </el-table>
    </div>
  </div>
</template>

<script>
import axios from "axios";
export default {
  data() {
    return {
      //月
      childmonth: sessionStorage.getItem("chooseMonth"),
      formData: null,
      fileName: "",
      fileList: [],
      // 弹窗参数
      headers: {
        token: sessionStorage.getItem("token")
      },
      ipAddress: window.axiosUrl.ip + "system/itemsFromExcel",
      tableList: [],
      tableTitle: 0
    };
  },
  created() {},
  methods: {
    handleRemove(file, fileList) {
      console.log(file, fileList);
    },
    upSuccess(response, file, fileList) {
      if (response.state === "1000") {
        this.$message.success("人才表导入成功!");
        this.tableList = response.result;
        console.log(response, file, fileList);
        this.tableTitle = Object.keys(this.tableList[0]).length - 1;
        this.fileName = file.name.substring(0, file.name.indexOf("."));
        this.formData = new FormData();
        this.formData.append("file", file.raw);
        this.$refs.upload.clearFiles();
      } else {
        this.tableList = [];
        this.tableTitle = 0;
        this.$message.success(response.message);
      }
    },
    handleExports() {
      let myObj = {
        method: "post",
        url: "system/downItemsFromExcel",
        fileName: "人才模板导出"
      };
      this.exportMethod(myObj);
    }
  }
};
</script>

<style lang="scss" scoped>
.main {
  width: 100%;
  height: 100%;
  .times {
    width: 100%;
    //   border-bottom: 1px solid #ccc;
    border-bottom: 15px solid #f5f5f5;
    height: 50px;
    position: fixed;
    z-index: 999;
    .times_wrap {
      width: 100%;
      height: 100%;
      display: flex;
      background: #fff;
      align-items: center;
      justify-content: flex-start;
      .button_out {
        margin-left: 20px;
      }
      .tips {
        margin-left: 20px;
        color: rgb(184, 10, 10);
        font-size: 15px;
        font-weight: bold;
      }
    }
  }
  .main_wrap {
    padding-top: 65px;
    width: 100%;
    display: flex;
    /deep/ .upload-demo {
      display: flex;
      justify-content: space-between;
      flex-flow: row-reverse;
      //      .el-upload-list {
      //   display: none !important;
      // }
    }
  }
}
</style>
